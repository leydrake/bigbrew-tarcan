...
if (isset($_POST["submit"])) {
    ...
    foreach ($categories as $category) {
        ...
        if (!empty($category)) {
            $query = "INSERT INTO categ_addson (category) VALUES('$category')";
...
if (isset($_POST["edit"])) {
    ...
    $oldCategoryQuery = "SELECT category FROM categ_addson WHERE id = $editCategoryId";
    ...
    if ($oldCategoryResult && mysqli_num_rows($oldCategoryResult) > 0) {
        ...
        $editQuery = "UPDATE categ_addson SET category = '$editCategoryName' WHERE id = $editCategoryId";
        mysqli_query($conn, $editQuery);

        $updateProductQuery = "UPDATE invent_addson SET category = '$editCategoryName' WHERE category = '$oldCategory'";
...
if (isset($_POST["delete_selected"])) {
    if (isset($_POST["selected_categories"]) && is_array($_POST["selected_categories"])) {
        foreach ($_POST["selected_categories"] as $selectedCategoryId) {
            $categoryQuery = "SELECT category FROM categ_addson WHERE id = $selectedCategoryId";
            $categoryResult = mysqli_query($conn, $categoryQuery);

            if ($categoryResult && mysqli_num_rows($categoryResult) > 0) {
                $categoryName = mysqli_fetch_assoc($categoryResult)['category'];

                $deleteProductsQuery = "DELETE FROM invent_addson WHERE category = '$categoryName'";
                mysqli_query($conn, $deleteProductsQuery);

                $deleteCategoryQuery = "DELETE FROM categ_addson WHERE id = $selectedCategoryId";
...
$searchQuery = "SELECT id, category, product_count FROM categ_addson WHERE category LIKE '%$searchTerm%'";
...
x
    ...
    .categ {
        ...
        width: 100%;
        height: 60px;
        background-color:rgb(235, 189, 116);
        display: flex;
        align-items: center;
        justify-content: center;
        box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
...
<div class="navigator">
        ...
        <div class="popup-overlay" id="popupOverlay">
            <div class="popup-content">
                ...
                <form action="" method="post" autocomplete="off" enctype="multipart/form-data">
                    ...
                    <button type="submit" class="add" name="submit">Add Add-Ons</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div>
        <button class="addito" onclick="showpopup()">Add Add-Ons</button>
                                ...
                                <td>
                                    ...
                                    <form action="" method="post">
                                        <div class="edit-name-container">
                                            ...
                                            <label for="edit_name">Edit Add-Ons:</label>
