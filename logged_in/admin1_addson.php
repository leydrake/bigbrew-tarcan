    ...
    try {
        ...
        if (!in_array($imageExtension, $validImageExtension)) {
            ...
            if (move_uploaded_file($tmpName, $uploadPath)) {
                $query = "INSERT INTO invent_addson (name, image, category, qty, price) 
                        VALUES ('$name', '$newImageName', '$category', $qty, '$price')";
                if (mysqli_query($conn, $query)) {
                    echo "<script>alert('Product Added Successfully'); document.location.href = 'admin1_addson.php';</script>";
                } else {
...
if (isset($_POST['delete_selected'])) {
    if (isset($_POST['delete']) && !empty($_POST['delete'])) {
        ...
        $deleteQuery = "DELETE FROM invent_addson WHERE id IN ($productIds)";
        if (mysqli_query($conn, $deleteQuery)) {
            echo "<script>alert('Selected products deleted successfully'); document.location.href = 'admin1_addson.php';</script>";
        } else {
            echo "Error: " . mysqli_error($conn);
        }
    } else {
...
$searchQuery = "SELECT id, name, image, category, qty, price, category_id FROM invent_addson WHERE name LIKE '%$searchTerm%' ORDER BY id DESC";
...
$categoryQuery = "SELECT DISTINCT category FROM categ_addson";
$categories = mysqli_query($conn, $categoryQuery);

if ($categoryFilter) {
    $searchQuery = "SELECT id, name, image, category, qty, price, category_id FROM invent_addson WHERE category = '$categoryFilter' ORDER BY id DESC";
} else {
    $searchQuery = "SELECT id, name, image, category, qty, price, category_id FROM invent_addson ORDER BY id DESC";
}
$products = mysqli_query($conn, $searchQuery);
...
if (isset($_POST['updateProduct'])) {
    ...
    $updateQuery = "UPDATE invent_addson SET name='$name', category='$category', qty=$qty, price='$price'";
    ...
    if (mysqli_query($conn, $updateQuery)) {
        echo "<script>alert('Product updated successfully'); document.location.href = 'admin1_addson.php';</script>";
    } else {
        echo "Error: " . mysqli_error($conn);
    }
}
...
<head>
    ...
    function toggleProfile() {
            ...
            if (profileContainer.classList.contains("open")) {
                ...
                body.style.marginTop = "340px";
                body.style.overflow = "hidden";

                nav.style.marginTop = "-175px";
    ...
    <div class="content-container">
        <div class="all">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-column">
                                ...
                                <select name="category" id="category" required>
                                    <?php
                                    $categoryQuery = mysqli_query($conn, "SELECT DISTINCT category FROM categ_addson");
            ...
            <div class="dynamic_flexible">
                <div class="view">
                    <h3 class="prod">Add-Ons Management</h3>
            ...
            <div class="popup-overlay" id="editPopup">
                <div class="popup-content">
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                            <label for="editName">AddsOn Name:</label>
