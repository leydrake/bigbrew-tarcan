...
if (isset($_POST["submit"])) {
    ...
    foreach ($categories as $category) {
        ...
        if (!empty($category)) {
            $query = "INSERT INTO categ_flavors (category) VALUES('$category')";
...
if (isset($_POST["edit"])) {
    ...
    $oldCategoryQuery = "SELECT category FROM categ_flavors WHERE id = $editCategoryId";
    ...
    if ($oldCategoryResult && mysqli_num_rows($oldCategoryResult) > 0) {
        ...
        $editQuery = "UPDATE categ_flavors SET category = '$editCategoryName' WHERE id = $editCategoryId";
        mysqli_query($conn, $editQuery);

        $updateProductQuery = "UPDATE invent_flavors SET category = '$editCategoryName' WHERE category = '$oldCategory'";
...
if (isset($_POST["delete_selected"])) {
    if (isset($_POST["selected_categories"]) && is_array($_POST["selected_categories"])) {
        foreach ($_POST["selected_categories"] as $selectedCategoryId) {
            $categoryQuery = "SELECT category FROM categ_flavors WHERE id = $selectedCategoryId";
            $categoryResult = mysqli_query($conn, $categoryQuery);

            if ($categoryResult && mysqli_num_rows($categoryResult) > 0) {
                $categoryName = mysqli_fetch_assoc($categoryResult)['category'];

                $deleteProductsQuery = "DELETE FROM invent_flavors WHERE category = '$categoryName'";
                mysqli_query($conn, $deleteProductsQuery);

                $deleteCategoryQuery = "DELETE FROM categ_flavors WHERE id = $selectedCategoryId";
...
$searchQuery = "SELECT id, category, product_count FROM categ_flavors WHERE category LIKE '%$searchTerm%'";
                                ...
                                <td>
                                    <form action="" method="post">
                                        <div class="edit-name-container">
                                            ...
                                            <label for="edit_name">Edit Flavor:</label>
